...
// Integral testing function: similar, with calculation of relative error
...
//                             Root Finding (root)
// ----------------------------------------------------------------------------------
int root_iterations = 0;  // Iteration counter (global, for option --iterations)
/*
Combined method for finding the root of equation F(x)=f(x)-g(x)=0
The method combines the secant method and Newton's method for efficient root finding.
At each step:
1. Compute the midpoint of the interval and the derivatives at this point
2. Depending on the signs of the derivatives, choose the update scheme
3. Apply the secant method for one boundary and Newton's method for the other
4. Repeat the process until the desired accuracy is reached
*/
double root(func f, func g, func df, func dg, double a, double b, double eps) {
    // Reset iteration counter for each call
    if (a > b) {
        double tmp = a;
        a = b;
        b = tmp;
    ...
    double fa = f(a) - g(a);
    double fb = f(b) - g(b);
    // Check for a root at the boundaries
    if (fa <= 0.000001 && fa > -0.000001) return a;
    if (fb <= 0.000001 && fb > -0.000001) return b;
    // Check the condition of root existence in the interval
    if (fa * fb > 0) {
        printf("No root on interval [%.6f, %.6f]\n", a, b);
        exit(1);
    }
    // Variables for midpoint, first and second derivatives of F
    double mid, Fp, Fpp;
    int case_num;
    // Main loop: shrink interval until its length <= eps
    do {
        root_iterations++;
        ...
        // 3) Secant step on the current interval [a,b]
        //    secant formula: x = (a*F(b) - b*F(a)) / (F(b) - F(a))
        double c_chord = (a * fb - b * fa) / (fb - fa);
        // 4) Newton step (tangent) at point d = (case==1 ? b : a)
    ...
    // Return the midpoint of the last interval as the root approximation
    return 0.5 * (a + b);
}
// ----------------------------------------------------------------------------------
...
double integral(func f, double a, double b, double eps2) {
    const double p = 1.0 / 15.0; // Runge's coefficient for Simpson's method
    int n = 2; // initial (minimum) even number of intervals
    ...
    while (1) {
        int n2 = n * 2;
        double h2 = (b - a) / n2;
        // sum f in new "odd" points of the 2n subdivision
        double sum_new_odd = 0.0;
        for (int i = 1; i < n2; i += 2) {
            // here i = 1,3,5,... — all odd nodes of the new grid
            sum_new_odd += f(a + i * h2);
        }
        // recompute I for 2n intervals:
        // old "odd" nodes become "even" (coefficient 2),
        // new ones remain "odd" (coefficient 4)
        ...
        sum_odd += sum_new_odd;  // all "odd" for 2n now included in sum_odd
        I_n = I_2n;
        n = n2;
        if (n > max_n) {
